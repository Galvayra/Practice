...
import numpy as np
from Practice.keras.keraspp import sfile, skeras
import os
...
class DataSet:
    ...
    def add_channels(self):
        ...
        if len(X.shape) == 3:
            ...
            print('channel is already included')
...
class Machine:
    ...
    def fit(self, epoch=10, batch_size=128, verbose=1):
        data = self.data
        model = self.model
        history = model.fit(data.X_train, data.Y_train, batch_size=batch_size, epochs=epoch, verbose=verbose, validation_data=(data.X_test, data.Y_test))

        return history

    def run(self, epoch=10, batch_size=128, verbose=1):
        data = self.data
        model = self.model
        fig = self.fig

        history = self.fit(epoch=4, batch_size=batch_size, verbose=verbose)
